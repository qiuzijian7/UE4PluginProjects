...
TSharedPtr<class SGraphPin> FBlueprintGraphPanelPinFactory4LookupTable::CreatePin(class UEdGraphPin* InPin) const
{
	...
	if (InPin->PinType.PinCategory == K2Schema->PC_Name)
	{
		...
		if (Outer->IsA(UK2Node_LookUpTable::StaticClass()))
		{
			const UK2Node_LookUpTable* CallFunctionNode = CastChecked<UK2Node_LookUpTable>(Outer);
			if (CallFunctionNode)
			{
